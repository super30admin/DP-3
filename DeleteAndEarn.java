...
//The new array we created , here for each index , we get the total to that index. Exampel if we had 3 2s in the input array , at index 2 we put 
// value 6.
//In the matrix array we build based on the previous optimal solutions. If we choose an element , we add the not choose value from previous element
...
class Solution {
    ...
    //new array where we store total of each digits from the input array at corresponding indices
    int n = getMax(nums)+1;
